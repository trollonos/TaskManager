...
{
    QMessageBox msgBox;
    int ret=msgBox.warning(this,tr("Предупреждение"),tr("Сохранить изменения?"),QMessageBox::Ok,QMessageBox::Cancel);
    if(ret==QMessageBox::Ok)
    {
        ...
        else
        {
            if(!_string_or_not(_taskpriority))
            {
                ...
                mb->warning(this,tr("Предупреждение"),tr("Неверное значение приоритетности, введите верное значение"),QMessageBox::Ok);
            }
            else
            {
                if(!window->is_string_valid(_taskname))
                {
                    QMessageBox *mb=new QMessageBox;
                    mb->warning(this,tr("Предупреждение"),tr("Недопустимое название, введите другое"),QMessageBox::Ok);
                }
            }
        }
     }
