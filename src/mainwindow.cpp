    ...
    ui(new Ui::MainWindow)
{
    ...
    if (!f.exists())
    {
        printf("Невозможно установить стили, файл не наёден\n");
    }
    else
    {
...
{
    QMessageBox msgBox;
    int ret=msgBox.warning(this,tr("Предупреждение"),tr("Вы действительно хотите удалить выбранную(ые) задачу(и) навсегда?"),QMessageBox::Ok,QMessageBox::Cancel);
...
{
    QMessageBox msgBox;
    int ret=msgBox.warning(this,tr("Предупреждение"),tr("Вы действительно хотите пометить выбранную(ые) задачу(и) как выполненные?"),QMessageBox::Ok,QMessageBox::Cancel);
     if(ret==QMessageBox::Ok){
    for(int i=0;i<=num-1;i++){
        if(box[i].first.isChecked()){
            qDebug()<<box[i].second;
...
void MainWindow::open_database(){
    ...
    sqldb.setDatabaseName("D:/Qt projects/TaskManager/TaskManager/database.db");
    sqldb.open();
...
{
    QMessageBox msgBox;
    int ret=msgBox.warning(this,tr("Предупреждение"),tr("Вы действительно хотите пометить выбранную(ые) задачу(и) как невыполненные?"),QMessageBox::Ok,QMessageBox::Cancel);
     if(ret==QMessageBox::Ok){
    for(int i=0;i<=num2-1;i++){
        if(box2[i].first.isChecked()){
            int day,month,year;
...
bool isfound=false;
    ...
    if(isfound==false){
        QMessageBox msgBox;
        int ret=msgBox.information(this,tr("Предупреждение"),tr("Ничего не найдено. Попробуйте изменить запрос"),QMessageBox::Ok);
    }
}
